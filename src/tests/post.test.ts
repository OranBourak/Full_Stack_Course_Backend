...
describe("Post test",()=>{
    ...
    test("Get /post:id - get post by ID ", async ()=>{
        ...
        expect(data.length).toBe(1);
        const ID = data[0]._id;
        const res2 = await request(app).get("/post/"+ID).set('Authorization', 'Bearer ' + testUser.accessToken);
    ...
    test("Put /post:id - update post by ID ", async ()=>{
        ...
        const res2 = await request(app).put("/post/"+ID).set('Authorization', 'Bearer ' + testUser.accessToken).send(updatedPost);
        expect(res2.statusCode).toBe(200);
        const res3 = await request(app).get("/post/"+ID).set('Authorization', 'Bearer ' + testUser.accessToken);
    ...
    test("Delete /post:id - delete post by ID ", async ()=>{
        ...
        const data = res.body;
        const ID = data[0]._id;
        const res2 = await request(app).delete("/post/"+ID).set('Authorization', 'Bearer ' + testUser.accessToken);
        expect(res2.statusCode).toBe(200);
        const res3 = await request(app).get("/post/"+ID).set('Authorization', 'Bearer ' + testUser.accessToken);
